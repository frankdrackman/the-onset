    ...
    <div class="detail">
      ...
      <div class="prose" style="max-width:76ch">
        ...
        ${section('Content honesty', [
          ['Counts are computed, never asserted', `Every number on the site is counted from the data file. The archive holds ${T.CATALOG.length} items — a representative sample, not a claimed archive. The brief states ~135 English articles as a labelled floor with the item-level LoHud crawl still pending, so the prototype does not assert 135.`],
          ['Bylines are placeholders', 'Physician bylines follow the wireframe’s own “Dr. Byline Name, MD” convention plus the real department. No ME clinician is named. The Person entity, credential strip and Find a Doctor <code>sameAs</code> are all wired and bind the moment the crawl supplies real names.'],
          ['Episode titles', 'Three episode titles are named in the brief and are carried verbatim — <em>Sleep Yourself Better</em>, <em>That Gut Feeling</em>, <em>At Your Convenience from Everywhere</em>. The other ten are not enumerated in either source, so they carry the wireframe’s own placeholder and are told apart by the season and episode meta.'],
          ['Every tie-break is cited in the data', 'The brief’s deterministic canonical tie-break is applied to all thirteen episodes, and each non-obvious case carries the rule that decided it as a comment in <code>episodes.mjs</code>: S1-E3 to Sleep Medicine, S1-E5 to Live Well with no entity claim, S1-E6 to Heart Care by the first-named-guest rule, the psychologist episodes to Behavioral Health, the two Internal Medicine episodes to Primary Care with human routing out to the pillars.'],
